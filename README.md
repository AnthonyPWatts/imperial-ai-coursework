# Imperial AI Coursework

Preparation and coursework workspace for the Imperial College Business School
Professional Certificate in Machine Learning and Artificial Intelligence.

## Purpose

This repo is a personal learning workspace for:

- refreshing linear algebra, calculus, probability and statistics
- practising Python, NumPy, pandas and scikit-learn
- completing notebook-based exercises
- preparing for ML methods and capstone work

## Structure

- `capstone/` — capstone project
- `capstone-prep/` — black-box optimisation and project preparation
- `course-labs/` — executable lab notebooks and environment checks
- `maths-refresh/` — pre-course maths notebooks implemented in Python
- `notebooks/` — my own notebooks on key topics, organised to match course modules
- `notes/` — programme maps, cheatsheets, setup notes and public resources
- `scratch/` — temporary experiments

## Current focus

- Linear algebra intuition with NumPy
- Jupyter workflow
- Python scientific stack
- Course/module mapping